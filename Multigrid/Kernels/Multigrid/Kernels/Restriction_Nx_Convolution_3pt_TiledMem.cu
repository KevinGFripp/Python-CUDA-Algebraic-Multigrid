extern "C" __global__ void Restriction_Nx_Convolution_3pt_TiledMem(
...
// Full size matrix performing 1D convolution -> Rx_2D = kron(Rx_1D,I_y)
    ...
    int tile_width = blockDim.x * 2 + 2; // halo of size 3pt - 1
    int tile_height = blockDim.y;

    // Map shared index, contiguous in x
    int s_idx = ty * tile_width + 2 * tx; //  i_coarse -> 2*i
    ...
    coarse[ic*Ny_2h + j] = 0.25f * f0 + 0.5f * f1 + 0.25f * f2;
